  ...
  <b-container>
    ...
    <b-row>
      ...
      <b-col sm="5" md="6" class="my-1">
        <b-form-group label="每页显示" label-cols-sm="6" label-cols-md="4" label-cols-lg="3" label-align-sm="right" label-size="sm" label-for="perPageSelect" class="mb-0">
...
export default {
  ...
  data () {
    return {
      checkedItemExBasic: [],
      itemExBasic: ['塑界者', '裂界者', '圣战', '救赎者', '猎人', '督军'],
      wantedAddedText: '',
      tabOptions: [],
      hortiCount: 0,
      hortiStation: [],
  ...
  methods: {
    hortiAnalysis (itemArray) {
      ...
      this.$message({
        duration: 2000,
        type: 'success',
        message: `已确认 ${this.hortiCount} 台花园工艺台, 總共有 ${this.statsTotal} 条工艺`
      })
    },
    clearHortiStation () {
      this.filteredRows = 0
          ...
          if (response.data.hasOwnProperty('error')) {
            ...
            switch (response.data.error.message) {
              case 'Resource not found':
                errorMessage = '请再次检查帐号名称是否正确！'
                break
              case 'Forbidden':
                errorMessage = '请再次检查 POESESSID 是否正确！'
                break
              default:
                errorMessage = `发生错误！${response.data.error.message}`
                break
            }
            vm.$message({
              type: 'error',
  ...
  computed: {
    ...
    operatorText () {
      return this.operatorMode ? '清楚工艺' : '更新工艺'
    }
  }
}
</script>

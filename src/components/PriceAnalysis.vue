  ...
  <div class="d-inline-flex p-2 bd-highlight">
    ...
    <table class="table table-striped" sticky-header v-if="collectionCurrency && !isLoading">
      <thead class="thead-dark">
        <tr>
          <th scope="col">前 {{ fetchResultPrice.length }} 笔价格分析
            <br>
            <span v-if="corruptedCount && corruptedCount === fetchResultPrice.length" style="color: lightpink;">{{ corruptedCount }} 笔已腐化</span>
            <b-button v-else-if="corruptedCount" @click="$emit('exclude')" size="sm" :variant="corruptedCount > 5 ? 'outline-danger' : 'outline-warning'">排除 {{ corruptedCount }} 笔已腐化</b-button>
            <div v-if="corruptedCount" style="padding: 1px 0px;"></div>
            <b-button v-if="fetchResultPrice.length <= 40 && fetchID.length >= 4 && calResultLength" @click="priceAnalysis(8)" :disabled="isCounting" size="sm" variant="outline-primary">再多搜 {{ calResultLength >= 40 ? 40 : calResultLength }} 笔价格</b-button>
          </th>
        </tr>
      </thead>
      <tbody @mouseleave="hoveredIndex = -1">
        <tr v-for="(item, index) in collectionCurrency" :key="index">
          <td :style="`opacity: ${hoveredIndex == index ? 1 : parseFloat(item.accountName.length / item.count)}; 
            ...
            / <b>{{ item.count }}</b>笔
            <span v-if="isPriceCollapse === false">（<b>{{ item.accountName.length }}</b>人标）</span>
            <b-icon-x-square v-show="hoveredIndex == index && $store.state.POESESSID" @click="addToBlackList(item.accountName)" v-b-tooltip.hover.right.v-secondary :title="`将 ${item.accountName.length} 用户加入黑名单`" style="cursor: pointer;"></b-icon-x-square>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  mounted() {
    //let vm = this
    //this.axios.post(`http://localhost:3031/static`, {
      //url: 'https://poe.game.qq.com/api/trade/data/static',
  ...
  methods: {
    ...
    priceAnalysis(indexLength) {
      ...
      this.axios.all(newFilterResult.map((element, index) => {
          let result = this.axios.post(`http://localhost:3031/fetch`, {
          url: this.baseUrl + '/api/trade/fetch/' + element + '?query=' + this.fetchQueryID,
          cookie: this.$store.state.POESESSID
          })
        return result
        }))
        ...
        .catch(function (error) {
          ...
          if (error.response.headers) {
            ...
            vm.$message({
              type: 'error',
              message: `被 Server 限制发送请求了，请等待 ${limitState} 秒后再重試`
            });
          }
        })
    },
    ...
    addToBlackList(accounts) {
      let vm = this
      this.$confirm(`是否将 ${accounts} ${accounts.length > 1 ? `等 ${accounts.length} 人` : ''}加入黑名单?`, '请再次确认', {
        ...
        this.axios.all(accounts.map(element => {
          ...
          .then(vm.axios.spread((...res) => {
            ...
            vm.$message({
              type: 'success',
              message: `已将 ${accounts.length} 人加入黑名单，下次查询时生效！`,
            });
          }))
          .catch(function (error) {
            console.log(error);
      ...
      collectionCurrency.forEach((collection, index) => {
        this.Currency.forEach(element => {
          if (collection.currency === element.id) {
            collectionCurrency[index].image = `https://poecdn.game.qq.com${element.image}`

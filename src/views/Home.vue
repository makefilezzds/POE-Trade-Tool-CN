...
<div class="home" ref="home">
  ...
  <b-alert v-if="isApiError" show variant="danger" style="margin-top: 5px;">
    <div>Ops! API 连接時似乎发生了一点错误...</div>
    <h4 style="padding-top: 6px;">{{ apiErrorStr }}</h4>
    <hr>
    <!--<div>若跳出提示 Code 503，表示腾讯服或国际服（Steam、Epic）官方应在关机维护中，请稍后再试</div>
    <div>若跳出提示 Code 401，表示POESESSID没有粘贴成功，请重新复制并粘贴</div>
    <div>若跳出提示 Code 403/404，表示程序访问API链接出现问题，请检查相关的网络访问</div>-->
    <div style="color:#3CB371">软件正常运行 -> 请粘贴腾讯服POESESSID</div>
    <div style="color:#FF0000">粘贴完成之后请按回车键（Enter）确认</div>
    <hr>
    <input
          :style="`width:${inputWidth}`"
          clearable
          v-model.trim = "formFields1.xs"
          maxlength = "32"
          class = "underline-input"
          placeholder = "此处粘贴腾讯服POESESSID"
          @keyup.enter = "updatePOESESSID"
        />
              <hr>
              <div style="color:#FF0000">粘贴完成之后请按回车键（Enter）确认</div>
              <div style="color:#3CB371">即使你粘贴错误的POESESSID，也会正常进入软件，但是腾讯服查询将不可用</div>
              <div style="color:#3CB371">教程点击下方按钮，请观看之后在进行操作</div>
              <hr>
      <b-button @click="openLink(`https://www.bilibili.com/video/BV1BF41177G7/`)" size="sm" variant="success">POESESSID教程</b-button> |
      <b-button @click="openLink(`https://www.bilibili.com/video/bv1hR4y1w7xA`)" size="sm" variant="success">软件使用教程</b-button>
              <hr>
    <countdown ref="countdown" :time="countTime" @end="handleCountdownEnd" :interval="100">
      <template slot-scope="props">
        <b-button v-if="isCounting" :disabled="isCounting" size="sm" variant="outline-danger">请等待 {{ props.seconds }}.{{ Math.floor(props.milliseconds / 50) }} 秒后重试</b-button>
        <!--<div v-else>
          <b-button @click="serverChange('国际服')" :disabled="isCounting" size="sm" variant="outline-danger">国际服（Steam、Epic）/玩家点我重试一次</b-button> /
          <b-button @click="serverChange('国服')" :disabled="isCounting" size="sm" variant="outline-danger">腾讯服玩家点我重试一次</b-button>
        </div>-->
      </template>
    </countdown>
  </b-alert>
  <div v-else>
    <b-container class="bv-example-row">
      <b-row class="lesspadding">
        <b-col align-self="center" style="padding-left: 6px !important;">
          <b-button v-b-toggle.collapse-1 size="sm" variant="outline-primary" @click.shift.middle="(clickCount > 5 && isGem) || handlePOESESSID ? clickOpen() : ''">搜索设定</b-button>
        </b-col>
        ...
        <b-card>
          <b-row class="lesspadding" style="padding-left: 2px;">
            ...
            <b-col sm="8" style="margin-left: 15px; padding-top: 5px;">
              <multiselect v-model="searchedText" :options="poedbTW.data" :custom-label="nameWithLang" :optionsLimit="10" :preserveSearch="true" :clearOnSelect="false" track-by="us" placeholder="请输入准备查询的物品中/英文字" selectLabel='' deselectLabel='' selectedLabel=''></multiselect>
            </b-col>
          ...
          <b-row class="lesspadding" style="padding-top: 5px; padding-left: 2px;">
            <b-col sm="3">
              <b-form-checkbox class="float-right" style="padding-top: 5px;" v-model="isPriceCollapse" :disabled="isCounting" switch :inline="false">
                <b v-b-tooltip.hover.right.v-secondary :title="`交易过滤条件：按账号折叠名单 (Collapse Listings by Account)`">按账号折叠</b>
              </b-form-checkbox>
            </b-col>
            <b-col sm="2">
              <b-form-checkbox class="float-right" style="padding-top: 5px;" v-model="corruptedSet.isSearch" :disabled="true" switch>已腐化</b-form-checkbox>
            </b-col>
            <b-col sm="3">
        ...
        <b-card>
          ...
          <b-row class="lesspadding">
            <b-col sm="3" style="padding-top: 6px;">
              <b-form-checkbox class="float-right" v-model="itemLevel.isSearch" @input="isItemLevelSearch" switch>物品等级</b-form-checkbox>
            </b-col>
          ...
          <b-row class="lesspadding" style="padding-top: 5px;">
            <b-col sm="3" style="padding-top: 6px;">
              <b-form-checkbox class="float-right" v-model="itemLinked.isSearch" @input="isLinkedSearch" switch>物品连接</b-form-checkbox>
            </b-col>
            <b-col sm="1" style="padding-top: 3px;">
    ...
    <b-container class="bv-example-row">
      <b-collapse :visible="isMap && isMapCollapse" class="mt-2">
        <b-card>
          <b-row class="lesspadding">
            <b-col sm="3" style="padding-top: 6px;">
              <b-form-checkbox class="float-right" v-model="areaLevel.isSearch" @input="isAreaLevelSearch" switch>区域等级</b-form-checkbox>
            </b-col>
            <b-col sm="1" style="padding-top: 3px;">
          ...
          <b-collapse :visible="raritySet.chosenObj.label !== '传奇' && isNormalMap">
            <b-row class="lesspadding" style="padding-top: 10px;">
              <b-col sm="4">
                <b-form-checkbox style="padding-left: 18px !important" v-model="mapCategory.isShaper" switch :inline="false">塑界领域</b-form-checkbox>
              </b-col>
              <b-col sm="3">
                <b-form-checkbox v-model="mapCategory.isElder" switch :inline="false">裂界领域</b-form-checkbox>
              </b-col>
              <b-col sm="3">
                <b-form-checkbox v-model="mapCategory.isCitadel" switch :inline="false">要塞领域</b-form-checkbox>
              </b-col>
              <b-col sm="2">
                <b-form-checkbox style="padding-left: 10px !important" v-model="mapCategory.isBlighted" switch :inline="false">菌潮地区</b-form-checkbox>
              </b-col>
            </b-row>
          </b-collapse>
          ...
          <b-collapse :visible="mapCategory.isCitadel && isNormalMap">
            <b-row style="padding-top: 8px;">
              <b-col sm="2"></b-col>
              <b-col sm="4" style="padding-left: 72px; padding-top: 5px;">
                <b-form-checkbox v-model="mapCitadelGuard.isSearch" @input="isMapCitadelGuardSearch" switch :inline="false">守卫</b-form-checkbox>
              </b-col>
    ...
    <b-alert v-if="isCounting && !isApiError" show variant="warning" style="margin-top: 5px;">
      <countdown ref="countdown" :time="countTime" @end="handleCountdownEnd" :interval="100">
        <template slot-scope="props">因 API 发送次数限制，请再等待：{{ props.seconds }}.{{ Math.floor(props.milliseconds / 100) }} 秒</template>
      </countdown>
    </b-alert>
    <!-- TODO: 词缀篩选 -->
    <b-container class="bv-example-row">
  ...
  <div v-if="!isSupported" style="padding:5px 30px;">
    <b-card header="问题反馈" border-variant="info" header-bg-variant="info" header-text-variant="white" align="center">
      ...
      <b-card-text style="padding-top: 20px;">
        <span>可点击报错物品信息字串反馈至 </span>
        <b-button @click="openLink(`https://qun.qq.com/qqweb/qunpro/share?_wv=3&_wwv=128&appChannel=share&inviteCode=1W4qo4V&from=181074&biz=ka#/pc`)" size="sm" variant="outline-primary" class="mb-2">
          POE流放之路（QQ频道）
        </b-button>
        <span> 感謝！</span>
      </b-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      searchStats: [], // 分析拆解后的物品詞綴陣列，提供使用者在界面勾选是否查詢及输入数值
      pseudoStats: [], // 综合
      explicitStats: [], // 随机屬性
      implicitStats: [], // 基底屬性
      fracturedStats: [], // 破裂
      ...
      craftedStats: [], // 已工艺
      crucibleStats: [], // 古神熔炉
      clusterJewelStats: [], // 星團珠宝附魔詞綴
      allocatesStats: [], // 項鍊涂油配置附魔詞綴
      forbiddenZoneStats: [], // 禁忌之火/血肉配置詞綴
      wrapStats: [],
      ...
      exJson: {
        "query": {
          ...
          "stats": [{
            "type": "and",
            "filters": [{
              "id": "enchant.stat_3948993189", // 附魔：星團珠宝(jewel.cluster) 基底詞綴ID
              "value": { // "增加的小天赋获得：#"
                "option": 16 // 1~53
              },
              'disabled': false
            }, {
              "id": "enchant.stat_2954116742", // 附魔：項鍊涂油 基底詞綴ID
              "value": { // "配置 #"
          ...
          "filters": {
            "misc_filters": {
              "filters": {
                ...
                "gem_level": { // 技能等级 isGemLevelSearch
                  "min": 10
                },
                "quality": { // 技能品质 isGemQualitySearch
                  "min": 10
                },
                "gem_alternate_quality": { // 寶石替代品质設定 gemQualityTypeInput
                  "option": "alternate", // any alternate
                  "option": "0", // superior(default) 精良的
                  "option": "1", // anomalous  异常
                  "option": "2", // divergent  分歧
                  "option": "3", // phantasmal 魅影
                },
              }
            },
            "type_filters": {
  ...
  created() {
    ...
    this.isGarenaSvr = this.storeServerString === '国服' ? true : false
  ...
  methods: {
    updatePOESESSID(e){
      this.$store.commit("setPOESESSID",e.target.value);
      this.getAllAPI(false);
    },
    initLocalStorage() {
    ...
    replaceString(string) {
      //let AmosePatchRE1 = /[^\u4e00-\u9fa5]+/g 
      //string = string.replace(AmosePatchRE1, '') //A大补丁正则
       //const regMatchBrackets = /\((.+?)\)/g // 取出括号内文字
       ...
       //string = string.replace(regEnglish, '')
    ...
    searchTrade: _.debounce(function (obj) {
      ...
      this.axios.post(`http://localhost:3031/trade`, {
        ...
        .then((response) => {
          ...
          if (JSON.stringify(this.searchJson) == JSON.stringify(this.searchJson_Def)) { // 嘗试修復有時搜尋会无法代入條件的 bug
            this.copyText = ''
          }
          this.status = ` 共 ${response.data.total} 笔符合 ${this.isPriceCollapse && response.data.total !== response.data.resultLength ? '- 报价已折叠' : ''}`
        ...
        .catch(function (error) {
          ...
          vm.issueText = `Version: V1.321.2, Server: ${vm.storeServerString}\n此次搜索异常！\n${errMsg}\n\`\`\`\n${vm.copyText.replace('稀有度: ', 'Rarity: ')}\`\`\``
          vm.itemsAPI()
    ...
    statsAPI() { // 詞綴 API
      //this.axios.post(`http://localhost:3031/stats`, {
        //url:"https://poe.game.qq.com/api/trade/data/stats",
        //cookie:this.$store.state.POESESSID,
      //})
        //.then((response) => {
          //let result = response.data.result
       let result = this.allStats.result
      result[0].entries.forEach((element) => { // 偽屬性
        let text = element.text
        if (text.indexOf('有房间：') > -1) { // 刪除 "有房間：" 字串
          text = text.substring(4, 20)
        }
        this.pseudoStats.push(text, element.id)
        if (text.indexOf('有日志') > -1) { // 探險日誌詞綴
          this.explicitStats.push(text, element.id)
        }
      })
      result[1].entries.forEach((element) => { // 隨機屬性
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
          } else if (text.indexOf(' (护盾)') > -1) { // stat_4253454700 刪除(護盾)字串
          text = text.substring(0, text.indexOf(' (护盾)'))
        }
        if (element.id === "explicit.stat_2460506030") { // 禁忌烈焰/血肉配置詞綴
          element.option.options.forEach((element) => {
      ...
      result[2].entries.forEach((element) => { // 固定屬性
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
          } else if (text.indexOf(' (护盾)') > -1) { // 刪除(護盾)字串
          text = text.substring(0, text.indexOf(' (护盾)'))
        }
        if (text.includes('\n')) { // 處理折行詞綴
          this.wrapStats.push(text)
      ...
      result[3].entries.forEach((element) => { // 破裂
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
          } else if (text.indexOf(' (护盾)') > -1) { // 刪除(護盾)字串
      ...
      result[4].entries.forEach((element) => { // 附魔
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
        }
      ...
      result[5].entries.forEach((element) => { // 災魘詞綴
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
        }
      ...
      result[6].entries.forEach((element) => { // 已工藝
        let text = element.text
        if (text.indexOf(' (部分)') > -1) { // 刪除(部分)字串
          text = text.substring(0, text.indexOf(' (部分)'))
        } else if (text.indexOf(' (护盾)') > -1) { // 刪除(護盾)字串
      ...
      /*result[7].entries.forEach((element, index) => { // 古神熔炉
        let text = element.text
        if (text.includes('\n')) { // 處理折行詞綴
          this.wrapStats.push(text)
        }
      ...
      result[8].entries.forEach((element, index) => { // 隱匿屬性
        let text = element.text
        if (text === '解密之') text = '影匿后缀'
        else if (text === '影匿的') text = '影匿前缀'
        this.explicitStats.push(text, element.id)
      })
        //})
        //.catch(function (error) {
    ...
    itemsAPI() { // 物品 API
      ...
      //this.axios.post(`http://localhost:3031/items`, {
        //url:'https://poe.game.qq.com/api/trade/data/items',
        //cookie:this.$store.state.POESESSID,
      //})
        //.then((response) => {
          //let result = response.data.result
          ...
          result[result.findIndex(e => e.id === "accessories")].entries.forEach((element) => { // "id": "accessories", "label": "飾品"
            ...
            // _.isUndefined(element.flags) == true 表示非传奇物品
            ...
            switch (accessoryIndex) {
              case 1: // 项链起始点 { "type": "碧珠护身符", "text": "碧珠护身符" }
                element.name = "项链"
                element.option = "accessory.amulet"
                this.equipItems.push(element)
                break;
              case 2: // 腰带起始点 { "type": "素布腰带", "text": "素布腰带" }
                element.name = "腰带"
                element.option = "accessory.belt"
                this.equipItems.push(element)
                break;
              case 3: // 戒指起始点 { "type": "裂隙戒指", "text": "裂隙戒指" }  
                element.name = "戒指"
                element.option = "accessory.ring"
                this.equipItems.push(element)
                break;
              case 4: // 飾品起始点 { "type": "盜賊飾品", "text": "盜賊飾品" }  
                element.name = "饰品"
          ...
          result[result.findIndex(e => e.id === "armour")].entries.forEach((element) => { // "id": "armour", "label": "護甲"
            ...
            // const basetype = ["黄金战甲", "异色鞋", "擒拿手套", "唤骨头盔", "黄金圣炎", "火灵箭袋"]
            if (_.isUndefined(element.flags)) {
              armourIndex += stringSimilarity.findBestMatch(element.type, basetype).bestMatch.rating === 1 ? 1 : 0
            }
            switch (armourIndex) {
              case 1: // 胸甲起始点 { "type": "黃金戰甲", "text": "黃金戰甲" }
                element.name = "胸甲"
                element.option = "armour.chest"
                this.equipItems.push(element)
                break;
              case 2: // 鞋子起始点 { "type": "異色鞋", "text": "異色鞋" }
                element.name = "鞋子"
                element.option = "armour.boots"
                this.equipItems.push(element)
                break;
              case 3: // 手套起始点 { "type": "擒拿手套", "text": "擒拿手套" }
                element.name = "手套"
                element.option = "armour.gloves"
                this.equipItems.push(element)
                break;
              case 4: // 頭部起始点 { "type": "喚骨頭盔", "text": "喚骨頭盔" }
                element.name = "头部"
                element.option = "armour.helmet"
                this.equipItems.push(element)
                break;
              case 5: // 盾牌起始点 { "type": "黃金聖炎", "text": "黃金聖炎" }
                element.name = "盾"
                element.option = "armour.shield"
                this.equipItems.push(element)
                break;
              case 6: // 箭袋起始点 { "type": "火靈箭袋", "text": "火靈箭袋" }
                element.name = "箭袋"
          ...
          result[result.findIndex(e => e.id === "flasks")].entries.forEach((element) => { // "id": "flasks", "label": "藥劑"
            ...
            switch (flasksIndex) {
              case 1: // 藥劑起始点 { "type": "小型複合藥劑", "text": "小型複合藥劑" }
                element.name = "药剂"
          ...
          //result[6].entries.forEach((element, index) => { // "label": "珠宝"
          result[result.findIndex(e => e.id === "jewels")].entries.forEach((element) => { // "id": "jewels", "label": "珠寶"
            ...
            switch (jewelIndex) {
              case 1: // 珠宝起始点 { "type": "催眠之眼珠宝", "text": "催眠之眼珠宝" }
                element.name = "珠宝"
          ...
          result[result.findIndex(e => e.id === "weapons")].entries.forEach((element) => { // "id": "weapons", "label": "武器"
            ...
            switch (weaponIndex) {
              case 1: // 爪起始点 { "type": "拳釘", "text": "拳釘" }
                element.name = "爪"
                element.option = "weapon.claw"
                element.weapon = "weapon.one" // "weapon.one" 单手武器
                this.equipItems.push(element)
                break;
              case 2: // 匕首起始点 { "type": "玻璃利片", "text": "玻璃利片" }
                element.name = "匕首"
                element.option = "weapon.dagger"
                element.weapon = "weapon.one"
                this.equipItems.push(element)
                break;
              case 3: // 单手斧起始点 { "type": "鏽斧", "text": "鏽斧" }
                element.name = "单手斧"
                element.option = "weapon.oneaxe"
                element.weapon = "weapon.one"
                this.equipItems.push(element)
                break;
              case 4: // 单手錘起始点 { "type": "朽木之棒", "text": "朽木之棒" }
                element.name = "单手锤"
                element.option = "weapon.onemace"
                element.weapon = "weapon.one"
                this.equipItems.push(element)
                break;
              case 5: // 单手劍起始点 { "type": "鏽劍", "text": "鏽劍" }
                element.name = "单手剑"
                element.option = "weapon.onesword"
                element.weapon = "weapon.one"
                this.equipItems.push(element)
                break;
              case 6: // 法杖起始点 { "type": "朽木法杖", "text": "朽木法杖" }
                element.name = "法杖"
              ...
              case 7: // { "type": "魚竿", "text": "魚竿" }
                ...
                this.equipItems.push(element)
                break;
              case 8: // 弓起始点 { "type": "粗製弓", "text": "粗製弓" }
                element.name = "弓"
                element.option = "weapon.bow"
                this.equipItems.push(element)
                break;
              case 9: // 長杖起始点 { "type": "朽木之幹", "text": "朽木之幹" }
                element.name = "长杖"
                element.option = "weapon.staff"
                element.weapon = "weapon.twomelee"
                this.equipItems.push(element)
                break;
              case 10: // 双手斧起始点 { "type": "石斧", "text": "石斧" }
                element.name = "双手斧"
                element.option = "weapon.twoaxe"
                element.weapon = "weapon.twomelee"
                this.equipItems.push(element)
                break;
              case 11: // 双手錘起始点 { "type": "朽木巨錘", "text": "朽木巨錘" }
                element.name = "双手锤"
                element.option = "weapon.twomace"
                element.weapon = "weapon.twomelee"
                this.equipItems.push(element)
                break;
              case 12: // 双手劍起始点 { "type": "鏽斑巨劍", "text": "鏽斑巨劍" }
                element.name = "双手剑"
          ...
          result[result.findIndex(e => e.id === "heistequipment")].entries.forEach((element) => { // "id": "heistequipment"
            ...
            switch (heistIndex) {
              case 1: // 劫盜裝備起始点 { "type": "鳗皮鞋底", "text": "鳗皮鞋底" }
                element.name = "赏金猎人装备"
      ...
      result[result.findIndex(e => e.id === "memoryline")].entries.forEach((element) => { // "id": "sentinel"
          if (_.isUndefined(element.flags)) {
          element.name = "回忆线"
    ...
    leaguesAPI() { // 聯盟 API
      let vm = this
      this.axios.post(`http://localhost:3031/leagues`, {
        url:this.baseUrl+'/api/trade/data/leagues',
        cookie:this.$store.state.POESESSID,
      })
        .then((response) => {
    ...
    gggAPI() {
      let vm = this
      const ignoreTyep = ["神諭之殿．神臨", "神諭之殿．歸徒", "神諭之殿．降師"] // 国际服的圖沒有這三種基底
      let tempMapBasic = []
      ...
      this.axios.get(`https://www.pathofexile.com/api/trade/data/items`, )
        .then((response) => {
          ...
          result[7].entries.forEach((element, index) => { // "label": "Maps"
            const basetype = ["Academy Map"] // 地圖起始点 { "type": "Academy Map", "text": "Academy Map" }
    ...
    //addAfterCopyText() {
      ...
      //this.$message({
       // duration: 2000,
       // type: 'success',
        //message: `已复制字串：${clipboard.readText().substring(0, 10)} ... ${this.wantedAddedText}`
      //});
    //},
    ...
    itemStatsAnalysis(itemArray, rarityFlag) {
      ...
      let clusterA = itemArray.findIndex((e) => e.indexOf('个珠宝插槽天赋') > -1) // 星團珠宝贅詞
      ...
      let itemStatStart = 0 // 物品随机詞綴初始位置
      let itemStatEnd = itemArray.length - 1 // 物品随机詞綴結束位置
      ...
      itemArray.forEach((element, index) => {
        ...
        // "--------" 字串前一筆資料若為基底詞或附魔詞或天灾詞，則不將此 index 視為詞綴結束点
        ...
        if (element.indexOf("增加的小天赋获得：") > -1 && element.indexOf("(enchant)") > -1) { // 有折行的星團珠宝附魔詞綴
          switch (true) {
            ...
            case element.indexOf("爪类攻击造成的击中和异常状态伤害提高 12%") > -1:
              ...
              spliceWrapStats(1, index)
              break;
            case element.indexOf("弓类伤害提高 12%") > -1:
            ...
            case element.indexOf("陷阱伤害提高 12%") > -1:
              ...
              spliceWrapStats(1, index)
              break;
            case element.indexOf("药剂给予的生命恢复提高 10%") > -1:
              itemArray[index] = `${itemArray[index]}\n药剂给予的魔力恢复提高 10%`
              spliceWrapStats(1, index)
              break;
            default:
        ...
        } else if (element.indexOf('只影响') > -1 && element.indexOf('圈内的天赋') > -1) { // 希望之絃 Thread of Hope 特殊判斷
          let areaStat = itemArray[index].substr(3, 1)
          itemArray[index] = `Only affects Passives in # Ring,${areaStat}`
        } else if (element.indexOf("焚界者物品") > -1 || element.indexOf("灭界者物品") > -1 || element.indexOf("Searing Exarch Item") > -1 || element.indexOf("Eater of Worlds Item") > -1) {
          spliceWrapStats(2, index + 1) // 忽略 3.17 新勢力詞綴
        }
        if (element === "--------" && !isEndPoint && itemStatStart && index > itemStatStart && itemStatEnd == itemArray.length - 1) { // 判斷随机詞墜結束点
          itemStatEnd = index
        }
      ...
      for (let index = itemStatStart; index < itemStatEnd; index++) {
        if (itemArray[index] !== "--------" && itemArray[index]) {
          ...
          if (itemArray[index].indexOf('(implicit)') > -1) { // 基底屬性
            ...
            tempStat[tempStat.length - 1].type = "分裂"
          } else if (itemArray[index].indexOf('(scourge)') > -1) { // 天灾
          ...
          } else if (itemArray[index].indexOf('(crafted)') > -1) { // 已工艺屬性
      ...
      tempStat.forEach((element, idx, array) => { // 比對詞綴，抓出随机数值與詞綴搜尋 ID
        let isStatSearch = false
        let bestIndex = (element.bestMatchIndex % 2 === 0) ? element.bestMatchIndex + 1 : element.bestMatchIndex // 處理判斷到英文詞綴的例外狀況，通常是季初有新詞綴尚未翻譯時才发生
        ...
        switch (true) { // 部分(Local)屬性判斷處理：若物品为武器，攻击属性是（部分）标识
          case statID.indexOf('stat_960081730') > -1 || statID.indexOf('stat_1940865751') > -1: // 附加 # 至 # 物理伤害 (部分)
            ...
            break;
          case statID.indexOf('stat_321077055') > -1 || statID.indexOf('stat_709508406') > -1: // 附加 # 至 # 火焰伤害 (部分)
            ...
            break;
          case statID.indexOf('stat_3531280422') > -1 || statID.indexOf('stat_2223678961') > -1: // 附加 # 至 # 混沌伤害 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('weapon') > -1) {
            ...
            break;
          case statID.indexOf('stat_1334060246') > -1 || statID.indexOf('stat_3336890334') > -1: // 附加 # 至 # 閃電伤害 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('weapon') > -1) {
            ...
            break;
          case statID.indexOf('stat_2387423236') > -1 || statID.indexOf('stat_1037193709') > -1: // 附加 # 至 # 冰冷伤害 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('weapon') > -1) {
          ...
          case statID.indexOf('stat_681332047') > -1 || statID.indexOf('stat_210067635') > -1: // 攻擊速度 (部分) 
            ...
            break;
          case statID.indexOf('stat_3593843976') > -1 || statID.indexOf('stat_55876295') > -1: // #% 的物理攻擊伤害偷取生命 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('weapon') > -1) {
            ...
            break;
          case statID.indexOf('stat_3237948413') > -1 || statID.indexOf('stat_669069897') > -1: // #% 所造成的物理攻擊伤害偷取魔力 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('weapon') > -1) {
            ...
            break;
            // 若物品是护甲，防御属性是（部分）标识
          case statID.indexOf('stat_2144192055') > -1 || statID.indexOf('stat_53045048') > -1: // # 点閃避值 (部分)
          ...
          case statID.indexOf('stat_2106365538') > -1 || statID.indexOf('stat_124859000') > -1: // 增加 #% 閃避值 (部分)
            ...
            break;
          case statID.indexOf('stat_809229260') > -1 || statID.indexOf('stat_3484657501') > -1: // # 点護甲 (部分)
            if (this.itemCategory.chosenObj.prop.indexOf('armour') > -1) {
          ...
          case statID.indexOf('stat_3489782002') > -1 || statID.indexOf('stat_4052037485') > -1: // # 最大能量護盾 (部分)
            ...
            break;
          case statID.indexOf('stat_3240073117') > -1 || statID.indexOf('stat_44972811') > -1: // # 处理国服两词缀相同翻译 "增加 #% 生命回复率"
          // stat_3240073117 Recovery rate: 腰带、护甲
          // stat_44972811 Regeneration rate: 项链、头手鞋
            ...
            break;
          case statID.indexOf('pseudo.pseudo_logbook') > -1: // 先祖密藏詞綴為偽屬性
            element.type = '综合'
            break;
          default:
            break;
        ...
        let randomMinValue = '' // 預設詞綴随机数值最小值為空值
        let randomMaxValue = '' // 預設詞綴随机数值最大值為空值
        let optionValue = 0 // 星團珠宝附魔 / 項鍊涂油配置 / 禁忌之火.血肉配置 的 ID
        ...
        if (statID === "enchant.stat_3948993189") {
          ...
          optionValue = parseInt(obj.ratings[obj.bestMatchIndex + 1].target, 10)
          apiStatText = `配置 涂油天赋：${obj.ratings[obj.bestMatchIndex].target}`
          ...
          optionValue = parseInt(obj.ratings[obj.bestMatchIndex + 1].target, 10)
          apiStatText = `禁断${statID === "explicit.stat_2460506030" ? '之火' : '之肉'}上有匹配的词缀则配置 ${obj.ratings[obj.bestMatchIndex].target}`
          ...
          apiStatText = `只影响『${areaStat}』圈内的天赋`
        } else {
          for (let index = 0; index < itemStatArray.length; index++) { // 比較由空格拆掉后的詞綴陣列元素
            ...
            if (!randomMinValue && itemStatArray[index] !== matchStatArray[index]) { // 物品詞綴最小值
              ...
              if (matchStatArray[index]) {
                if (matchStatArray[index].indexOf('，#') > -1) { // 處理随机数值在'，'后的詞綴(无法用空格符號 split)
                  let tempStat = itemStatArray[index].substring(itemStatArray[index].indexOf('，') + 1)
                  randomMinValue = parseFloat(tempStat.replace(/[+-]^\D+/g, ''))
                } else if (matchStatArray[index].indexOf('：#') > -1) { // 處理随机数值在'：'后的詞綴(无法用空格符號 split)
                  let tempStat = itemStatArray[index].substring(itemStatArray[index].indexOf('：') + 1)
                  randomMinValue = parseFloat(tempStat.replace(/[+-]^\D+/g, ''))
                }
              }
        ...
        let isNegativeStat = false // API 詞綴只有"增加"，但物品可能有"减少"詞綴相關處理
        if (apiStatText.includes('增加') && itemStatText.includes('减少')) {
          apiStatText = apiStatText.replace('增加', '减少')
          isNegativeStat = true
        }
         if (this.itemCategory.chosenObj.prop === 'logbook' && itemStatText.includes('区域内有') && (itemStatText.includes('梅德维德') || itemStatText.includes('沃拉纳') || itemStatText.includes('乌崔德') || itemStatText.includes('欧罗什'))) { // 探險日誌探險頭目相關處理
          isStatSearch = true
          statID = "implicit.stat_3159649981"
          apiStatText = itemStatText.replace('(implicit)', '')
          if (itemStatText.indexOf("梅德维德") > -1) optionValue = 1
          else if (itemStatText.indexOf("沃拉纳") > -1) optionValue = 2
          else if (itemStatText.indexOf("乌崔德") > -1) optionValue = 3
          else if (itemStatText.indexOf("欧罗什") > -1) optionValue = 4
        }
        if (statID === "enchant.stat_3086156145" || statID === "explicit.stat_1085446536") { // cluster jewel analysis
          isStatSearch = true
          this.isStatsCollapse = true
          let tempValue = randomMinValue
          switch (randomMinValue) { // 附加天赋数判斷
            case 4:
        ...
        if (elementalResistanceTotal && idx === array.length - 1) {
          this.searchStats.unshift({ // 若該裝備有抗性詞，增加综合至詞綴最前端
        ...
        if (spellDamageTotal && idx === array.length - 1) {
          this.searchStats.unshift({ // 計算法术伤害综合
    ...
    templeStatsAnalysis(itemArray) {
      if (itemArray.indexOf('堵塞的房间：') > -1) // 只判斷是否有該房間，受阻房間可用彈藥房炸通
        itemArray.splice(itemArray.indexOf('堵塞的房间：'), 1)
      ...
      tempStat.forEach((element) => {
        let statID = element.ratings[element.bestMatchIndex + 1].target // 詞綴ID
        let apiStatText = element.bestMatch.target // API 抓回來的詞綴字串
        const regapiStatText = /\[{1}(.+?)]|[\(（][^\)）]+[A-Za-z]+[\)）]|Ⅰ+|Ⅲ+|Ⅱ+|\s/g
        let apiStatText2 = apiStatText.replace(regapiStatText, '')
        let isLevel3Room = (apiStatText2.indexOf('等阶:3') > -1)
    ...
    mirroredStatsAnalysis(itemArray) {
      ...
      let itemStatStart = 6
      let itemStatEnd = itemArray.findIndex(e => e.indexOf('在私人地图装置中使用此项，开启通往卡兰德拉湖的传送门。') > -1) - 2 // 鏡像碑牌詞綴結束點
      ...
      tempStat.forEach((element, idx) => {
        ...
        if (difficultyLevel > 4) { // 只保留階級十以上映像
          this.searchStats.push({
            "id": statID,
            "text": apiStatText,
            "option": '',
    ...
    compassStatsAnalysis(itemArray) {
      ...
      let itemStatEnd = itemArray.findIndex(data => data === "右键点击该物品，再左键点击虚空石，把变成物品的六分仪词缀添加给虚空石。") - 2 //  羅盤詞綴結束點

      itemArray.forEach((element, index) => { // 處理遊戲內述敘與 API 敘述不一致之詞綴
        if (element.indexOf("该地图会出现【伊恩哈尔】") > -1) { // enchant.stat_3187151138: 遊戲內敘述 "區域含有埃哈"、詞綴 API 敘述 "區域包含 # (大師)"，需輸入 option
          itemArray[index] = `区域内包含# (大师)`
          optionValue = 2
        } else if (element.indexOf("该地图会出现【阿尔瓦】") > -1) {
          itemArray[index] = `区域内包含# (大师)`
          optionValue = 3
        } else if (element.indexOf("该地图会出现【尼克】") > -1) {
          itemArray[index] = `区域内包含# (大师)`
          optionValue = 5
        } else if (element.indexOf("该地图会出现【琼】") > -1) {
          itemArray[index] = `区域内包含# (大师)`
          optionValue = 6
        } else if (element.indexOf("地图内裂隙属于乌尔尼多") > -1) { // enchant.stat_1542416476: 遊戲內敘述 "區域中的裂痕為烏爾尼多"、詞綴 API 敘述 "區域中裂痕屬於 #"，需輸入 option
          itemArray[index] = `区域内的裂隙属于 #`
          optionValue = 1
        } else if (element.indexOf("地图内裂隙属于索伏") > -1) {
          itemArray[index] = `区域内的裂隙属于 #`
          optionValue = 2
        } else if (element.indexOf("地图内裂隙属于托沃") > -1) {
          itemArray[index] = `区域内的裂隙属于 #`
          optionValue = 3
        } else if (element.indexOf("地图内裂隙属于艾许") > -1) {
          itemArray[index] = `区域内的裂隙属于 #`
          optionValue = 4
        } else if (element.indexOf("地图内裂隙属于夏乌拉") > -1) {
          itemArray[index] = `区域内的裂隙属于 #`
          optionValue = 5
        } else if (element.indexOf("地图内的庄园至少有一片") > -1) { // enchant.stat_832377952: 遊戲內敘述 "區域中的豐收含有至少 1 個 # 色作物"、詞綴 API 敘述 "區域中豐收至少含有 1 個 # 作物"，需輸入 option
          if (element.indexOf("紫") > -1) optionValue = 1
          else if (element.indexOf("黃") > -1) optionValue = 2
          else if (element.indexOf("蓝") > -1) optionValue = 3
          itemArray[index] = `区域内的庄园至少有一片 # 庄稼`
        }
      });
      ...
      tempStat.forEach((element, idx) => {
        ...
        let itemStatText = itemDisplayStats[idx] // 物品上的詞綴字串
         if (statID == 'enchant.stat_3187151138') { // 處理在 UI 上顯示的"區域含有大師"詞綴，與遊戲內一致
          switch (optionValue) {
            case 2:
              apiStatText = '该地图会出现【伊恩哈尔】'
              break;
            case 3:
              apiStatText = '该地图会出现【阿尔瓦】'
              break;
            case 5:
              apiStatText = '该地图会出现【尼克】'
              break;
            case 6:
              apiStatText = '该地图会出现【琼】'
              break;
          }
        } else if (statID == 'enchant.stat_1542416476') { // 處理在 UI 上顯示的"區域中的裂痕為"詞綴，與遊戲內一致
          switch (optionValue) {
            case 1:
              apiStatText = '地图内裂隙属于乌尔尼多'
              break;
            case 2:
              apiStatText = '地图内裂隙属于索伏'
              break;
            case 3:
              apiStatText = '地图内裂隙属于托沃'
              break;
            case 4:
              apiStatText = '地图内裂隙属于艾许'
              break;
            case 5:
              apiStatText = '地图内裂隙属于夏乌拉'
              break;
          }
        } else if (statID == 'enchant.stat_832377952') { // 處理在 UI 上顯示的"區域中的豐收含有至少 1 個 # 色作物"詞綴，與遊戲內一致
          switch (optionValue) {
            case 1:
              apiStatText = '地图内的庄园至少有一片紫庄稼'
              break;
            case 2:
              apiStatText = '地图内的庄园至少有一片黄庄稼'
              break;
            case 3:
              apiStatText = '地图内的庄园至少有一片蓝庄稼'
              break;
          }
        }
    ...
    findBestStat(text, stats) { // 物品上原先詞綴 與 原先詞綴数值用 '#' 取代的兩種字串皆判斷並取最符合那一筆
      let floatValue = []
      let reference = []
      ...
      // 加权
      modifiedObj.bestMatch.rating += 0.1;

      reference.push(originalObj, modifiedObj)
      floatValue.push(originalObj.bestMatch.rating, modifiedObj.bestMatch.rating)

      if (text.includes('减少')) { // 處理物品上原先詞綴包含 '减少' 的情況：因部分詞綴於 api 中只顯示 '增加'，会造成詞綴誤判
        text = text.replace('减少', '增加')
    ...
    itemAnalysis(item, itemArray, matchItem) {
      ...
      switch (matchItem.option) { // 藥劑、劫盜裝備、釋界之邀会自動搜尋該基底
        case 'flask':
        case 'heistequipment':
          this.itemLevel.isSearch = true // 藥劑及劫盜裝備增加物等篩选
          this.isItemLevelSearch()
        case 'map.invitation':
    ...
    isItemLevelSearch() {
      if (!this.itemLevel.isSearch && this.isSearchJson) {
        delete this.searchJson.query.filters.misc_filters.filters.ilvl // 刪除物品等级 filter
      } else if (this.itemLevel.isSearch && this.isSearchJson) {
        this.searchJson.query.filters.misc_filters.filters.ilvl = { // 增加物品等级最小值 filter
    ...
    gemQualityTypeInput() { // 寶石替代品质設定 
      if (!this.gemQualitySet.isSearch && this.gemQualitySet.chosenObj.prop && this.isSearchJson) {
        delete this.searchJson.query.filters.misc_filters.filters.gem_alternate_quality // 刪除寶石替代品质 filter
    ...
    isExBasicSearch() {
      if (!this.itemExBasic.isSearch && this.itemExBasic.chosenObj.prop && this.isSearchJson) {
        delete this.searchJson.query.filters.misc_filters.filters[this.itemExBasic.chosenObj.prop] // 刪除勢力选項
      } else if (this.itemExBasic.isSearch && this.itemExBasic.chosenObj.prop && this.isSearchJson) {
        this.searchJson.query.filters.misc_filters.filters[this.itemExBasic.chosenObj.prop] = { // 增加目前选擇的勢力
          "option": "true"
        }
      }
    },
    ...
    exBasicChange(value) {
      let exSearchItem = !this.itemExBasic.chosenObj.prop ? '' : this.itemExBasic.chosenObj.prop // 前一個搜尋的勢力选項
      if (this.isSearchJson && this.searchJson.query.filters.misc_filters.filters.hasOwnProperty(exSearchItem)) {
        delete this.searchJson.query.filters.misc_filters.filters[exSearchItem] // 刪除前一個勢力选項
      }
      if (this.isSearchJson) {
        this.searchJson.query.filters.misc_filters.filters[value.prop] = { // 增加目前选擇的勢力
          "option": "true"
        }
      }
    ...
    mapAnalysis(item, itemArray, Rarity) {
      ...
      this.raritySet.isSearch = true
      this.isRaritySearch()
      let mapPos = item.indexOf('地图阶级:') > -1 ? item.substring(item.indexOf('地图阶级:') + 5) : 0 // 地圖階級截斷字串
      let areaPos = item.indexOf('地区等级:') > -1 ? item.substring(item.indexOf('地区等级:') + 5) : 0 // 地區等級截斷字串
        if (!areaPos)
        areaPos = item.indexOf('区域等级:') > -1 ? item.substring(item.indexOf('区域等级:') + 5) : 0 // 區域等級截斷字串
      if (mapPos) {
      ...
      if (Rarity === "传奇") { //传奇地圖
        ...
        if (item.indexOf('该地图被【裂界守卫：奴役】占据 (implicit)') > -1) { // 尊師守衛地圖
        ...
        } else if (item.indexOf('该地图被【裂界守卫：寂灭】占据 (implicit)') > -1) {
          this.mapElderGuard.chosenObj = {
            label: "裂界守卫:寂灭",
            prop: "2"
          }
          this.mapElderGuard.isSearch = true
          this.isMapElderGuardSearch()
        } else if (item.indexOf('该地图被【裂界守卫：约束】占据 (implicit)') > -1) {
          this.mapElderGuard.chosenObj = {
            label: "裂界守卫:约束",
            prop: "3"
          }
          this.mapElderGuard.isSearch = true
          this.isMapElderGuardSearch()
        } else if (item.indexOf('该地图被【裂界守卫：净世】占据 (implicit)') > -1) {
          this.mapElderGuard.chosenObj = {
            label: "裂界守卫:净世",
            prop: "4"
          }
      ...
      } else if (item.indexOf('地图上有巴兰的要塞 (implicit)') > -1) { // 壁壘守衛地圖
      ...
      } else if (item.indexOf('地图上有维利塔妮娅的要塞 (implicit)') > -1) {
        this.mapCategory.isCitadel = true
        this.mapCitadelGuard.chosenObj = {
          label: "救赎者维利塔妮娅",
          prop: "2"
        ...
        this.mapCitadelGuard.isSearch = true
        this.isMapCitadelGuardSearch()
      } else if (item.indexOf('地图上有阿尔赫兹敏的要塞 (implicit)') > -1) {
        this.mapCategory.isCitadel = true
        this.mapCitadelGuard.chosenObj = {
          label: "猎人阿尔赫兹敏",
          prop: "3"
        ...
        this.mapCitadelGuard.isSearch = true
        this.isMapCitadelGuardSearch()
      } else if (item.indexOf('地图上有狄洛克斯的要塞 (implicit)') > -1) {
        this.mapCategory.isCitadel = true
        this.mapCitadelGuard.chosenObj = {
          label: "督军狄洛克斯",
          prop: "4"
        ...
        this.mapCitadelGuard.isSearch = true
        this.isMapCitadelGuardSearch()
      } else if (item.indexOf('菌潮') > -1 || item.indexOf('Blighted') > -1) {
    ...
    isMapLevelSearch() {
      if (!this.mapLevel.isSearch && this.isSearchJson) {
        delete this.searchJson.query.filters.map_filters.filters.map_tier // 刪除地图阶级 filter
      } else if (this.mapLevel.isSearch && this.isSearchJson) {
        this.searchJson.query.filters.map_filters.filters.map_tier = { // 指定地图阶级最小 / 最大值 filter
    ...
    isGemLevelSearch() {
      if (!this.gemLevel.isSearch && this.isSearchJson) {
        delete this.searchJson.query.filters.misc_filters.filters.gem_level // 刪除技能等级 filter
      } else if (this.gemLevel.isSearch && this.isSearchJson) {
        this.searchJson.query.filters.misc_filters.filters.gem_level = { // 指定技能等级最小 / 最大值 filter
    ...
    isGemQualitySearch() {
      if (!this.gemQuality.isSearch && this.isSearchJson) {
        delete this.searchJson.query.filters.misc_filters.filters.quality // 刪除技能品质 filter
      } else if (this.gemQuality.isSearch && this.isSearchJson) {
        this.searchJson.query.filters.misc_filters.filters.quality = { // 指定技能品质最小 / 最大值 filter
    ...
    searchedCopyText(text) {
      ...
      this.$message({
        duration: 2000,
        type: 'success',
        message: `已复制文字：${clipboard.readText()}`
      });
    },
  },
  watch: {
    copyText: function () {
      ...
      if (this.isCounting) {
        ...
        this.$message({
          duration: 2000,
          type: 'error',
          message: `请等待限制间隔倒数完毕后再次按下 Ctrl+C`
        });
        return
      }
      this.resetSearchData();
      ...
      //if (item.indexOf('地图阶级:') > -1 && item.indexOf('在私人地图装置中使用可以前往该地图。只能被使用一次。') > -1) { // 地圖搜尋// 類地圖搜尋
      if (item.indexOf('物品类别: 异界地图') > -1 || item.indexOf('贤主之邀：') > -1 || item.indexOf('物品类别: 契约') > -1 || item.indexOf('物品类别: 蓝图') > -1 || item.indexOf('物品类别: 地图(Maps)') > -1) 
      { 
        this.mapAnalysis(item, itemArray, Rarity)
      } else if ((Rarity === "稀有" || Rarity === "传奇") && item.indexOf('右键点击后加入你的猎魔笔记。') > -1) { // 獸獵（物品化怪物）
        let monstersCount = 0
      ...
      } else if (Rarity === "传奇" && item.indexOf('在丹恩的试验室里') === -1) { // 传奇道具
        if (item.indexOf('古典传奇') > -1) {
          this.raritySet.chosenObj = {
            label: "古典传奇",
            prop: 'uniquefoil'
          }
        ...
        if (item.indexOf('未鉴定') === -1) { // 已鑑定传奇
          ...
          if (searchName.indexOf('精良的') > -1) { // 未鉴定的品质传奇物品
        ...
        if (item.indexOf('可以在私人地图装置中使用，在当天开启一扇通向阿佐亚特神庙的传送门。') > -1) { // 史記房間判斷
          ...
          this.isStatsCollapse = true
          return
        } else if (item.indexOf('在私人地图装置中使用此项，开启通往卡兰德拉湖的传送门。') > -1) { // 鏡像碑牌判斷
        ...
        if (item.indexOf("异常 ") > -1) { // 替代品质判斷
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '1'
          this.gemQualitySet.chosenObj.label = '异常'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        } else if (item.indexOf("分歧 ") > -1) {
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '2'
          this.gemQualitySet.chosenObj.label = '分歧'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        } else if (item.indexOf("魅影 ") > -1) {
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '3'
          this.gemQualitySet.chosenObj.label = '魅影'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        } else if (item.indexOf("异常(Anomalous) ") > -1) {
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '1'
          this.gemQualitySet.chosenObj.label = '异常'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        } else if (item.indexOf("分歧(Divergent) ") > -1) {
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '2'
          this.gemQualitySet.chosenObj.label = '分歧'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        } else if (item.indexOf("魅影(Phantasmal) ") > -1) {
          this.gemQualitySet.isSearch = true
          this.gemQualitySet.chosenObj.prop = '3'
          this.gemQualitySet.chosenObj.label = '魅影'
          this.gemBasic.chosenG = searchName.split(' ')[1]
        ...
        if (item.indexOf('瓦尔：') > -1) { // 瓦爾技能
          const regSetStringgem = /[\(（][^\)）]+[A-Za-z]+[\)）]/g
          let vaalPos = item.substring(item.indexOf('瓦尔：'))
          vaalPos = vaalPos.replace(regSetStringgem, '')
        ...
        if (item.indexOf('品质: +') > -1) {
          let quaPos = item.substring(item.indexOf('品质: +') + 5) // 品质截斷字串 (包含'品质: +'前的字串全截斷)
          let quaPosEnd = quaPos.indexOf('% (augmented)') // 品质定位点
          minQuality = parseInt(quaPos.substring(0, quaPosEnd).trim(), 10)
        }
        if (!this.gemQualitySet.isSearch) { // 若沒有替代品质，則搜尋技能品质
          this.gemQuality.isSearch = true
        }
        this.gemQuality.min = minQuality
        this.isGemQualitySearch()
      } else if (Rarity === "普通" && !this.isItem) {
        // } else if (Rarity === "普通" && (item.indexOf('透過聖殿實驗室或個人') > -1 || item.indexOf('可以使用於個人的地圖裝置來增加地圖的詞綴') > -1 || item.indexOf('放置兩個以上不同的徽印在地圖裝置中') > -1 || item.indexOf('你必須完成異界地圖中出現的全部六種试煉才能進入此區域') > -1 || item.indexOf('擊殺指定数量的怪物后会掉落培育之物') > -1 || item.indexOf('將你之前祭祀神壇保存的怪物加入至該地圖的祭祀神壇中') > -1 || item.indexOf('使用此物品開啟前往无悲憫與同情之地的時空之門') > -1 || item.indexOf('在個人地圖裝置使用此物品開啟譫妄異域時空之門') > -1 || item.indexOf('地圖裝置來使用此物品以前往進入瓦爾寶庫') > -1)) {
        // 地圖碎片、裂痕石、徽印、聖甲蟲、眾神聖器、女神祭品、培育器、浸血碑器、釋界之令、幻像異界、瓦爾遺鑰
        this.searchJson.query.type = this.replaceString(searchName)
        if (item.indexOf('右键点击该物品，再左键点击虚空石，把变成物品的六分仪词缀添加给虚空石。') > -1) { // 充能的羅盤
        ...
        this.itemsAPI()
        this.issueText = `Version: V1.321.2\n尚未支持该道具\n\`\`\`\n${this.copyText.replace('稀 有 度:', 'Rarity: ')}\`\`\``
  ...
  computed: {
    inputWidth() {
      if (this.formFields1.xs.length <= 10) return "180px";
      if (this.formFields1.xs.length <= 32)
        return 20 + 16 * this.formFields1.xs.length + "px";
      else {
        return "500px";
      }
    },
    ...
    isNormalMap() {
      return this.mapBasic.chosenM.indexOf('贤主之邀：') === -1 && this.mapBasic.chosenM.indexOf('蓝图：') === -1 && this.mapBasic.chosenM.indexOf('契约：') === -1
    }
  },
}
